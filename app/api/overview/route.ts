...
export async function GET(request: Request) {
  ...
  const planRequirements = await all(db, `SELECT pref.student_id,pref.rank preference_rank,ur.university,ur.department,ur.course_name,ur.recommendation_type,ur.note,'exact' match_level FROM student_preferences pref JOIN university_requirements ur ON ur.admissions_year=pref.admissions_year AND ur.university=pref.university AND ur.department=pref.department JOIN students s ON s.id=pref.student_id WHERE s.entrance_year=? UNION ALL SELECT pref.student_id,pref.rank preference_rank,ur.university,ur.department,ur.course_name,ur.recommendation_type,ur.note,'same_department' match_level FROM student_preferences pref JOIN university_requirements ur ON ur.admissions_year=pref.admissions_year AND ur.department=pref.department AND ur.university<>pref.university JOIN students s ON s.id=pref.student_id WHERE s.entrance_year=? UNION ALL SELECT pref.student_id,pref.rank preference_rank,'' university,tr.department_group department,tr.course_name,'recommended' recommendation_type,tr.note,'track' match_level FROM student_preferences pref JOIN track_requirements tr ON tr.admissions_year=pref.admissions_year AND tr.department_group=pref.department JOIN students s ON s.id=pref.student_id WHERE s.entrance_year=?`, entranceYear, entranceYear, entranceYear);
  ...
  return json({ entranceYear, students, plans, planCourses, planPreferences, planRequirements, planReviews, issues, files, rounds, curriculum, courseDescriptions, audit, comparisons, validations });
}
