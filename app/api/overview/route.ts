...
export async function GET(request: Request) {
  ...
  if (session.role === 'student') {
    ...
    const courseDescriptions = student ? await all(db, `SELECT s.*, sel.display_mode FROM course_description_selections sel JOIN course_description_sources s ON s.id=sel.selected_source_id WHERE sel.entrance_year=? UNION ALL SELECT 'education_pdf_' || eg.id id,eg.criteria_year entrance_year,eg.course_name,eg.course_nature overview,eg.content_structure learning_content,eg.course_nature,eg.related_careers || CASE WHEN eg.related_departments<>'' THEN ' / ' || eg.related_departments ELSE '' END related_careers,NULL,NULL,eg.selection_type,eg.hierarchy note,'official' source_kind,eg.source_document source_name,eg.criteria_year source_year,eg.created_at,eg.created_at,'official' display_mode FROM education_office_course_guides eg JOIN reference_uploads ru ON ru.id=eg.upload_id AND ru.active=1 LEFT JOIN course_description_selections sel ON sel.entrance_year=eg.criteria_year AND sel.course_name=eg.course_name WHERE eg.criteria_year=? AND sel.course_name IS NULL ORDER BY course_name`, (student as { entrance_year: number }).entrance_year, (student as { entrance_year: number }).entrance_year) : [];
