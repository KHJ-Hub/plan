...
export async function GET(request: Request) {
  ...
  if (session.role === 'student') {
    ...
    const trackCourseGuides = student ? await all(db, `SELECT * FROM academic_track_course_guides WHERE entrance_year=? AND active=1 ORDER BY academic_track,priority,target_grade,target_semester,course_name`, (student as { entrance_year: number }).entrance_year) : [];
    const schoolBookEntries = student ? await all(db, `SELECT sb.* FROM school_course_book_entries sb JOIN reference_uploads ru ON ru.id=sb.upload_id AND ru.active=1 WHERE sb.entry_year=? ORDER BY sb.target_grade,sb.target_semester,sb.subject_name`, (student as { entrance_year: number }).entrance_year) : [];
    ...
    return json({ student, profile, preferences, rounds, plans, reviews, official, curriculum, trackCourseGuides, schoolBookEntries, courseDescriptions, requirements });
  }
  ...
  const trackCourseGuides = await all(db, `SELECT * FROM academic_track_course_guides WHERE entrance_year=? AND active=1 ORDER BY academic_track,priority,target_grade,target_semester,course_name`, entranceYear);
  const schoolBookEntries = await all(db, `SELECT sb.* FROM school_course_book_entries sb JOIN reference_uploads ru ON ru.id=sb.upload_id AND ru.active=1 WHERE sb.entry_year=? ORDER BY sb.academic_year DESC,sb.target_grade,sb.target_semester,sb.subject_name`, entranceYear);
  ...
  return json({ entranceYear, students, plans, planCourses, planPreferences, planRequirements, planReviews, issues, files, rounds, curriculum, trackCourseGuides, schoolBookEntries, courseDescriptions, referenceUploads, schoolCourseGuides, educationOfficeCourseGuides, universityReferenceEntries, trackReferenceEntries, audit, comparisons, validations });
}
