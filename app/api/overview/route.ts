...
export async function GET(request: Request) {
  ...
  if (session.role === 'student') {
    ...
    const courseDescriptions = student ? await all(db, `SELECT s.*, sel.display_mode FROM course_description_selections sel JOIN course_description_sources s ON s.id=sel.selected_source_id WHERE sel.entrance_year=? UNION ALL SELECT 'school_guide_' || scg.id id,scg.criteria_year entrance_year,scg.course_name,scg.overview,'' learning_content,'' course_nature,'' related_careers,scg.target_grade recommended_grade,scg.target_semester recommended_semester,NULL selection_type,scg.note,'official' source_kind,'학교 공식 과목 안내 자료' source_name,scg.criteria_year source_year,scg.created_at,scg.created_at,'official' display_mode FROM school_course_guides scg JOIN reference_uploads ru ON ru.id=scg.upload_id AND ru.active=1 LEFT JOIN course_description_selections sel ON sel.entrance_year=scg.criteria_year AND sel.course_name=scg.course_name WHERE scg.criteria_year=? AND scg.overview<>'' AND sel.course_name IS NULL ORDER BY course_name`, (student as { entrance_year: number }).entrance_year, (student as { entrance_year: number }).entrance_year) : [];
    const requirements = await all(db, `SELECT ur.* FROM university_requirements ur LEFT JOIN reference_uploads ru ON ru.id=ur.source_upload_id JOIN student_preferences sp ON sp.student_id=? AND sp.admissions_year=ur.admissions_year AND sp.university=ur.university AND sp.department=ur.department WHERE ur.source_upload_id IS NULL OR ru.active=1 ORDER BY ur.university, ur.department`, session.studentId);
  ...
  return json({ entranceYear, students, plans, planCourses, planPreferences, planRequirements, planReviews, issues, files, rounds, curriculum, courseDescriptions, referenceUploads, schoolCourseGuides, audit, comparisons, validations });
}
