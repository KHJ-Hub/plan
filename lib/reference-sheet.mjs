...
export function matrixCoursesFromRow(headers, mapping, row, fallbackArea = '') {
  ...
  return headers.map((header, index) => ({ header, index })).filter(({ header, index }) => {
    ...
    return header && !header.startsWith('열 ') && !mappedColumns.has(index) && value && !/^[-–—]+$/.test(value);
  }).map(({ header, index }) => {
    const parts = header.split('/').map((part) => part.trim()).filter(Boolean);
    return { courseName: parts.at(-1) || header, subjectArea: parts.length > 1 ? parts.slice(0, -1).join(' > ') : fallbackArea, universities: String(row[index] ?? '').trim(), recommendationType: 'recommended' };
  });
}
